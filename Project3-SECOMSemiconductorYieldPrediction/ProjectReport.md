...
1. Missing Value Handling: Compute the missing fraction for each feature.
    ...
    $\displaystyle x_{ij,\text{imputed}} = \operatorname{median}{x_{kj}: x_{kj} \text{ observed}}$
...
$\mathbf{h}^{(1)} = \operatorname{ReLU}(W^{(1)}\mathbf{x} + \mathbf{b}^{(1)}), \qquad$
$\mathbf{h}^{(2)} = \operatorname{ReLU}(W^{(2)}\mathbf{h}^{(1)} + \mathbf{b}^{(2)})$,
